...
- **Service Layer** (Business Logic): Manages complex logic such as calculating transaction fees, verifying OTPs, and enforcing ACID compliance for money transfers
- **Repository Layer** (JDBC): Direct interaction with PostgreSQL using PreparedStatements to prevent SQL injection
- **Model Layer** (POJOs): Represents database entities including Users, Wallets, and Transactions
...
- Java JDK 17 or higher (Project targets JDK 22)
- Maven
...
| POST | `/api/auth/login` | No | Login (Triggers OTP) |
| POST | `/api/auth/verify-otp` | No | Verify OTP & Get Token |
| POST | `/api/txn/deposit` | Yes | Deposit money |
| POST | `/api/txn/airtime` | Yes | Buy Airtime |
